...
def get_all_urls(thread_name, url, internal_urls, external_urls, all_urls, timeout, ignore_extensions):
    ...
    for a_tag in soup.findAll("a"):
        ...
        all_urls.append(href)
    return [url.replace("javascript://Open('", '').replace("')", '') for url in internal_urls], [url.replace("javascript://Open('", '').replace("')", '') for url in external_urls], [url.replace("javascript://Open('", '').replace("')", '') for url in all_urls], response.text
def crawl(thread_name, urls, external, timeout, ignore_extensions):
    crawl_data = {}
    try:
        for url in urls:
            start_url = url
...
if __name__ == "__main__":
    ...
    if not data.write:
        data.write = f"{date.today()} {strftime('%H_%M_%S', localtime())}.json"
    if data.timeout:
    ...
    for index, url_division in enumerate(url_divisions):
        threads.append(pool.apply_async(crawl, (f"Thread {index+1}", url_division, data.external, data.timeout, data.ignore_extensions)))
    ...
    with open(f"{data.write}", 'w') as file:
        json.dump(crawl_data, file)
